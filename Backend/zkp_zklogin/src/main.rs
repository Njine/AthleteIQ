...
fn keccak_to_eth_address_hint(hash: &[GF2], address: &mut [GF2]) -> Result<(), Error> {
    ...
    let private_key =
        SecretKey::from_slice(&hash_bytes).expect("Failed to generate valid private key");
...
fn main() {
    ...
    // inputs
    let sub = "109322328489232325";
    let email = "tqwe23223232323@gmail.com";
    let aud = "6132232323275t3k6v.apps.googleusercontent.com";
    let salt = "a2c581f30f023432323232375fc1f910f8d1d371b87d8e";
    ...
    println!("Circuit validation result: {}", output[0]);
    ...
    compile_result.layered_circuit.serialize_into(&mut writer)?;

    let file = File::create("witness.txt")?;
    let mut writer = BufWriter::new(file);
    witness.serialize_into(&mut writer)?;

    println!("Circuit and witness files created in the project directory.");
    Ok(())
}
